...
do { //do-while loop to determine the point match, if winner wasn't determined during the first round
    ...
    if(playerDiceRoll == pointMatch ){ //if total rolled is greater that the point match the player wins
        ...
        *balance = bettingAmount * 3;
        ...
        printf("Computer rolled %i, winning the match\n", computerDiceRoll);
        *balance = *balance - bettingAmount;
        return LOSE;
        ...
        *balance = bettingAmount * 3;
        return WON;
            ...
            *balance = *balance - bettingAmount;
            return LOSE;
    }else if (playerDiceRoll == 2 || playerDiceRoll == 3 || playerDiceRoll == 12){ //if a '2', '3', '12' is rolled the player loses
        *balance = *balance - bettingAmount;
        return LOSE;
    } else if (computerDiceRoll == 2 || computerDiceRoll == 3 || computerDiceRoll == 12){ //if a player rolls either an 2, 3 or 12 they lose
            printf("Congratulations computer rolled %i losing the match and you WIN!\n", computerDiceRoll);
            *balance = bettingAmount * 3;
            return WON;
    }else{
